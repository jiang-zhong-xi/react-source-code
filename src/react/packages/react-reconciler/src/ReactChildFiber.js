...
function ChildReconciler(shouldTrackSideEffects) {
  function deleteChild(returnFiber: Fiber, childToDelete: Fiber): void {
    ...
    /*
      如果当前没有effect，那么当前的effect即作为firstEffect也是lastEffect。
      如果当前有effect，那么lastEffect和firstEffect的最后一个nextEffect是一个fiber
      所以直接把当前fiber添加到lastEffect的nextEffect即可。
      总结：lastEffect的目的就是维护一个最新的fiber
    */
    if (last !== null) {
  ...
  */
  function placeChild(
    ...
    if (current !== null) {
      ...
      /*
        一旦旧的fiber对应的index比lastPlacedIndex当前值大就把这个值赋给lastPlacedIndex否则就把新的fiber标记为Placement
      */
  ...
  function placeSingleChild(newFiber: Fiber): Fiber {
    /* 
      单child的情况比较简单，只需对要插入的新children做替换
    */
  ...
  // 如果旧的节点fiber为null或者不是HostText则创建新的fiber否则根据旧的节点fiber克隆一个fiber并返回
  ...
  /*
    如果元素类型和key值都匹配上了并且oldFiber不为空则返回重用的fiber否则返回null
  */
  function updateSlot(
    returnFiber: Fiber,
    oldFiber: Fiber | null,
  ...
  function reconcileChildrenArray(
    ...
    // resultingFirstChild是最终的fiber链表
    let resultingFirstChild: Fiber | null = null;
    // 当前遍历的新fiber的前一个fiber
  ...
  /* 将组件解析为fiber，其中type和elementType存储组件 */
  function reconcileSingleElement(
    ...
    /* 
      ...
      2. 如果已有的child和新的child的key相同（都没有key也是相同）
        ...
        b. 如果不是同类型的，把当前fiber和同级fiber添加到sideEffect队列并且打标签为“DELETE”
    */
    while (child !== null) {
  ...
  /* 
    这个API将用解析的side-effect给children打标签。
    当我们传递children和parent的时候它们会被添加到side-effect列表。
  */
  // This API will tag the children with the side-effect of the reconciliation
  // itself. They will be added to the side-effect list as we pass through the
  // children and the parent.
  function reconcileChildFibers(
    returnFiber: Fiber, // 当前fiber作为子fiber的return属性的值
    currentFirstChild: Fiber | null, // 当前fiber的第一个子fiber
    newChild: any, // 新的虚拟DOM
    expirationTime: ExpirationTime,
  ): Fiber | null {
    /* 
      这个函数不是递归。
      如果顶层item是一个数组，我们把它看作一系列的children而不是fragment。
      另一方面嵌套的数组被视为fragment节点。递归发生在常规流。
      像数组一样处理顶层没有key的fragment，这导致<>{[...]}</> 和 <>...</>之间的模糊,我们把模糊的例子视为一样。
    */
...
/*
  基于当前的fiber.child新建一个fiber并返回，新旧fiber通过alternate互相指向
*/
