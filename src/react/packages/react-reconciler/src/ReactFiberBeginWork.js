...
export function reconcileChildren(
  ...
  if (current === null) {
    /* 
      如果这是一个尚未render的新组件，我们不会通过应用最小的side-efffects来更新它的子组件集。
      相反，我们将在子元素被渲染之前将它们(side-effects)全部添加到子元素中。
      这意味着我们可以通过不跟踪side-effects来优化这一解析方案。 
    */
    ...
    /* 
      如果当前child和在执行中的work一样，那么就意味着还未在这些children上开启任何work，
      于是我们使用clone算法复制当前的所有children。
      如果有执行中的work，那么这个点上就没有作用了所以把它抛出。
    */
...
/* 
  1. 首先确定是否需要更新，如不需要则直接bailout(已完成的退出)
  2. 如果需要更新，则重新执行render获取最新的reactElement
  3. 重新接续并返回workInProgress.child继续迭代
*/
...
function pushHostRootContext(workInProgress) {
  ...
  // 把active的root fiber和root DOM推入栈
  pushHostContainer(workInProgress, root.containerInfo);
}
/* 
  1. 更新ROOT state（包括element）
  2. 将element解析为fiber
  3. 返回fiber
*/
...
/*
  该函数一方面保证当前fiber正常退出（因为没有要执行的work），
  另一方面判断child是否有要执行的work，如何没有，返回null退出该分支的任务执行，如果有
  执行cloneChildFibers
*/
...
function beginWork(
  ...
  if (current !== null) {
    ...
    if (oldProps !== newProps || hasLegacyContextChanged()) {
      // 如果props或者context改变了，把fiber标记为已经执行完work
      ...
      didReceiveUpdate = false;
      /* 
        当前fiber没有要执行的work，不进入下面的解析阶段，但是需要记录一下 
      */
