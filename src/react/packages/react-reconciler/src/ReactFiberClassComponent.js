...
// 执行getDerivedStateFromProps(nextProps, prevState),将返回的新state 和prevState合并后返回
...
// 执行class的constructor函数，初始化实例属性，将class实例和fiber建立关联，
...
function updateClassInstance(
  ...
  const shouldUpdate =
    ...
    /* 
    很奇妙的设计，workInProgress.effectTag |= 一个tag值
    如果下次workInProgress.effectTag 和这个值执行workInProgress.effectTag &则返回的布尔值是true，
    这样effectTag就可以存储多个标识
    */
  if (shouldUpdate) {
    // In order to support react-lifecycles-compat polyfilled components,
    // Unsafe lifecycles should not be invoked for components using the new APIs.
